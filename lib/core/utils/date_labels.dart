...
class DateLabels {
  ...
  /// '9:00am' — lowercase, no space, for compact inline timestamps (e.g. a
  /// routine's "marked done at" time in a history list). Strips any
  /// whitespace character, not just a literal ' ' — some ICU data formats
  /// the AM/PM gap with a narrow no-break space (U+202F) instead.
  static String compactTimeLabel(DateTime date) =>
      timeLabel(date).replaceAll(RegExp(r'\s+'), '').toLowerCase();
